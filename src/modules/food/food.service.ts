...
export class FoodService extends DB {
  ...
  async createOne(
    ...
    return db
      .collection('restaurant')
      .updateOne({ id: idRestaurant }, { $addToSet: { foods: body } });
  }
  ...
  async updateOne(
    ...
    return db.collection('restaurant').updateOne(
      { id: idRestaurant, 'foods.id': id },
      {
        $set: {
          'foods.$.name': body['name'],
          'foods.$.price': body['price'],
          'foods.$.id_restaurant': body['id_restaurant'],
          'foods.$.id_category': body['id_category'],
          'foods.$.details': body['details'],
          'foods.$.foods': body['foods'],
        },
      },
    );
  }
  ...
  async deleteOne(idRestaurant: number, id: number): Promise<UpdateResult> {
    ...
    return db
      ...
      .updateOne({ id: idRestaurant }, { $pull: { foods: { id: id } } });
  }
}
