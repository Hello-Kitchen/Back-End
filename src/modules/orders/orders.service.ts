 ...
 */
  async findFoodByIdsWithParam(idRestaurant: number, id: number[]): Promise<mongoose.mongo.WithId<mongoose.AnyObject>> {
    const db = this.getDbConnection();
 ...
 */
  async markFoodOrderedReady(idRestaurant: number, idFoodOrdered: number): Promise<mongoose.mongo.WithId<mongoose.AnyObject>> {
    const db = this.getDbConnection();
 ...
 * 
 * @param {string} restaurantId The ID of the restaurant containing the order.
