...
export class JwtStrategy extends PassportStrategy(Strategy) {
  constructor() {
    super({
      jwtFromRequest:
        ExtractJwt.fromAuthHeaderAsBearerToken() /**< @brief Extracts the token from the Authorization header */,
      ignoreExpiration: false /**< @brief Expired tokens are not allowed */,
      secretOrKey:
        'secret' /**< @brief Must match the secret defined in `JwtModule.register` */,
    });
  }
  ...
  async validate(payload: any) {
    ...
    }; /**< @brief Returns the user ID and username from the token payload */
  }
}
